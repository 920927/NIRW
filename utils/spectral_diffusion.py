...
class SPD(nn.Module):
	...
	def SPD_quantization(self, image_yuv_dct, scale_factor, table1, table2, round_func=torch.round):

		luminance_quant_tbl = (table1 * scale_factor).round().to(image_yuv_dct.device).clamp(min=1).repeat(
			image_yuv_dct.shape[2] // 8, image_yuv_dct.shape[3] // 8)

		chrominance_quant_tbl = (table2 * scale_factor).round().to(image_yuv_dct.device).clamp(min=1).repeat(
			image_yuv_dct.shape[2] // 8, image_yuv_dct.shape[3] // 8)
	...
	def SPD_reverse_quantization(self, q_image_yuv_dct, scale_factor, table1, table2):

		luminance_quant_tbl = (table1 * scale_factor).round().to(q_image_yuv_dct.device).clamp(min=1).repeat(
			q_image_yuv_dct.shape[2] // 8, q_image_yuv_dct.shape[3] // 8)

		chrominance_quant_tbl = (table2 * scale_factor).round().to(q_image_yuv_dct.device).clamp(min=1).repeat(
			q_image_yuv_dct.shape[2] // 8, q_image_yuv_dct.shape[3] // 8)
	...
	def round_ss(self, x):
		# print("x",x.min(),x.max())        
		cond = torch.tensor((torch.abs(x) < 0.5), dtype=torch.float).to(x.device) #<0.5； <1
		outx = cond * (x ** 3) + (1 - cond) * x
		# print("outx",outx.min(),outx.max())          
		return outx
	...
	def forward(self, image_and_cover):
		...
		randratio = torch.rand(luminanceT.shape)+0.4      
		table1 = luminanceT*randratio
		table2 = chrominanceT*randratio
		# quantization
		image_quantization = self.SPD_quantization(image_dct, self.scale_factor, table1, table2, self.round_ss)

		# reverse quantization
		image_quantization = self.SPD_reverse_quantization(image_quantization, self.scale_factor, table1, table2)
